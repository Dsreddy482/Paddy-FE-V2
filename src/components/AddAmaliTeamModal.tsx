...
interface AddAmaliTeamModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  loadingId: number;
...
export default function AddAmaliTeamModal({
  isOpen,
  onClose,
  onSuccess,
  loadingId
}: AddAmaliTeamModalProps) {
  const [formData, setFormData] = useState<Partial<AmaliTeam>>({
    loadingId,
    amaliTeamName: '',
    loadingType: 'potha',
    ratePerBag: 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Failed to add amali team:', error);
      alert('Failed to add amali team');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60]">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">Add Amali Team</h2>
